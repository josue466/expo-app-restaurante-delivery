...
export default function DetailScreen() {
  ...
  const { pizzas, setCart } = useApp();
  const router = useRouter();

  const parsed = JSON.parse(itemJson || "{}");
  const item = { ...parsed, img: pizzas.find(p => p.id === parsed.id)?.img ?? null };
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.hero}>
        ...
        {item.img
          ? (
            <View style={s.heroImgContainer}>
              <Image source={{ uri: item.img }} style={s.heroImg} resizeMode="cover" />
            </View>
          )
          : <Text style={s.heroEmoji}>{categoryEmoji(item.category ?? "")}</Text>
        }
